...
public class ProductService {
    ...
    @Resource
    private ProductBaseInfoController productBaseInfoController;
    ...
    public BuyResponseBo checkProduct(BuyBo buyBo) {
        ...
        int i=productController.addControlProduct(controlProductDto);
        if(i!=1){
            buyResponseBo.setOrderReturnCode(false);
            buyResponseBo.setErrorDetail("插入产品控制表失败");
        }else{
            ProductBaseInfoDto productBaseInfoDto = productBaseInfoController.findById(buyBo.getProductId());
